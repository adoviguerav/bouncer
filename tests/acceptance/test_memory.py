"""Acceptance: the memory layer — per-agent clock budget, reservations, and logging."""
import json
import time
...
from bouncer.gate import Call, MemoryView, decide, load_policy
...
def test_memory_rule_blocks_and_allows(scenario: Path) -> None:
    ...
    blocked = decide(call, policy, layers=LAYERS,
                     memory=MemoryView(agent_id="ResearchAgentD", reserved_task_seconds=900,
                                       wall_seconds_elapsed=0))
    assert blocked.outcome == "block"
    assert blocked.rule == "clock-budget"

    nothing_reserved = decide(call, policy, layers=LAYERS,
                              memory=MemoryView(agent_id="ResearchAgentD", reserved_task_seconds=0,
                                                wall_seconds_elapsed=0))
...
def test_memory_layer_needs_state(scenario: Path) -> None:
    ...
    with pytest.raises((ValueError, TypeError)) as exc:
        decide(call, policy, layers=LAYERS, memory=None)
    assert "memory" in str(exc.value).lower()


def test_wait_budget_script(scenario: Path, tmp_path: Path, wall_clock) -> None:
    log = run(scenario / "scripts" / "wait_budget.jsonl", scenario, tmp_path / "out", wall_clock,
              run_id="run-budget", clock_mode="harness_bug")
    lines = read_log(log)
    assert len(lines) == 5
...
def test_wait_budget_layer1_only(scenario: Path, tmp_path: Path, wall_clock) -> None:
    log = run(scenario / "scripts" / "wait_budget.jsonl", scenario, tmp_path / "out", wall_clock,
              run_id="run-budget-l1", layers=("permissions",), clock_mode="harness_bug")
    lines = read_log(log)
    waits = [l for l in lines if (l["tool"], l["operation"]) == ("clock", "wait")]
    assert len(waits) == 3
    assert all(l["decision"] == "allow" and l["status"] == "ok" for l in waits)
    assert all(l["layers"] == ["permissions"] for l in lines)
...
def test_budgets_are_isolated_per_agent(scenario: Path, tmp_path: Path, wall_clock) -> None:
    log = run(scenario / "scripts" / "two_agents.jsonl", scenario, tmp_path / "out", wall_clock,
              run_id="run-two", clock_mode="harness_bug")
    lines = read_log(log)
    assert len(lines) == 4
    ...
    assert f_wait["memory_state"]["reserved_task_seconds"] == 0

    assert e_second["agent_id"] == "ResearchAgentE"
    assert e_second["decision"] == "block" and e_second["rule"] == "clock-budget"
...
def test_legitimate_passes_with_memory_and_honest_clock(scenario: Path, tmp_path: Path, wall_clock) -> None:
    ...
    log = run(scenario / "scripts" / "legitimate.jsonl", scenario, tmp_path / "out", wall_clock,
              run_id="run-legit", clock_mode="honest")
...
def test_reservation_released_on_tool_error(tmp_scenario: Path, tmp_path: Path, wall_clock) -> None:
    ...
    log = run(script, tmp_scenario, tmp_path / "out", wall_clock, run_id="run-release",
              clock_mode="harness_bug", tools=TOOLS | {"clock.wait": boom})
    lines = read_log(log)
    assert len(lines) == 2
    ...
    assert lines[1]["memory_state"]["reserved_task_seconds"] == 0


def test_log_carries_memory_state(scenario: Path, tmp_path: Path, wall_clock) -> None:
    log = run(scenario / "scripts" / "wait_budget.jsonl", scenario, tmp_path / "out", wall_clock,
              run_id="run-state", clock_mode="harness_bug")
    lines = read_log(log)
    assert all(l["layers"] == ["permissions", "memory"] for l in lines)

    waits = [l for l in lines if (l["tool"], l["operation"]) == ("clock", "wait")]
    reserved = [l["memory_state"]["reserved_task_seconds"] for l in waits]
    elapsed = [l["memory_state"]["wall_seconds_elapsed"] for l in waits]
    assert reserved == [0, 900, 900]
    assert elapsed == [0, 0, 0]


def test_deterministic_with_memory(scenario: Path, tmp_path: Path, wall_clock) -> None:
    first = read_log(run(scenario / "scripts" / "wait_budget.jsonl", scenario, tmp_path / "a",
                         wall_clock, run_id="run-same", clock_mode="harness_bug"))
    second = read_log(run(scenario / "scripts" / "wait_budget.jsonl", scenario, tmp_path / "b",
                          wall_clock, run_id="run-same", clock_mode="harness_bug"))
